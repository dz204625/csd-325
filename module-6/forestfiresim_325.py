        ...
        for x in range(forest['width']):
            for y in range(forest['height']):
                ...
                if forest[(x, y)] == EMPTY and random.random() <= GROW_CHANCE:
                    ...
                    for ix in range(-1, 2):
                        for iy in range(-1, 2):
                            # Fire spreads to neighboring trees, but not to the lake:
...
def createNewForest():
    ...
    lake_width = 30  # Lake Width
    lake_height = 10  # Lake Height
    lakeX = (WIDTH // 2) - (lake_width // 2)
    lakeY = (HEIGHT // 2) - (lake_height // 2)
    
    for x in range(WIDTH):
        for y in range(HEIGHT):
            if lakeX <= x < lakeX + lake_width and lakeY <= y < lakeY + lake_height:
                forest[(x, y)] = LAKE  
